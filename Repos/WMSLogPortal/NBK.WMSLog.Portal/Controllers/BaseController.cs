...
namespace NBK.WMSLog.Portal.Controllers
{
    public class BaseController : Controller
    {
        ...
        public CoreQuery LoginCoreQuery
        {
            get
            {
                ...
                if (CurrentUser != null)
                {
                    query.ParmsObj = new { SessionKey = CurrentUser.UserId, SystemId = (int)SystemEnum.WMS };
                }
                query.AddParameter("systemId", ((int)SystemEnum.WMS).ToString());
                return query;
            }
        }
    ...
    /// <summary>
    /// 赋值用户登陆信息标签，action 接收到 baseDto时自动对相关userid等信息赋值
    /// </summary>

...
namespace WMSBussinessApi.Utility.Extend
{
    public static class JTransform
    {
        ...
        private static void JAutoCreateMap(Type sourceType, Type destinationType)
        {
            foreach (var perProperty in sourceType.GetProperties())
            {
                ...
                if (dProperty != null)
                {
                    if (!perProperty.PropertyType.GetTypeInfo().IsValueType && perProperty.PropertyType != typeof(string)) //非string的引用类型
                    {
                        if (perProperty.PropertyType.GetTypeInfo().IsGenericType && perProperty.PropertyType.GenericTypeArguments.First().GetTypeInfo().IsClass && perProperty.PropertyType.GenericTypeArguments.First() != typeof(string))
                        {
                            if (dProperty.PropertyType.GetTypeInfo().IsGenericType && dProperty.PropertyType.GenericTypeArguments.First().GetTypeInfo().IsClass && dProperty.PropertyType.GenericTypeArguments.First() != typeof(string))
                            {
                                JAutoCreateMap(perProperty.PropertyType.GenericTypeArguments.First(), dProperty.PropertyType.GenericTypeArguments.First());
                            }
                        }
                        else if (perProperty.PropertyType.GetTypeInfo().IsClass && dProperty.PropertyType.GetTypeInfo().IsClass) // 非泛型的属性,比如 MyType P3 {get;set;}
                        {
        ...
        public static object JCoverageTo(this object source, object destination, params string[] nonCoveragePropertyNames)
        {
            ...
            foreach (var perProperty in ot.GetProperties())
            {
                ...
                if (dProperty != null)
                {
                    ...
                    //非string的引用类型
                    if (!perProperty.PropertyType.GetTypeInfo().IsValueType && perProperty.PropertyType != typeof(string))
                    {
                        //泛型
                        if (perProperty.PropertyType.GetTypeInfo().IsGenericType && dProperty.PropertyType.GetTypeInfo().IsGenericType)
                        {
                            //泛型参数 为引用
                            if (perProperty.PropertyType.GenericTypeArguments.First().GetTypeInfo().IsClass && dProperty.PropertyType.GenericTypeArguments.First().GetTypeInfo().IsClass)
                            {
                                //泛型参数 为非string的引用
                                if (perProperty.PropertyType.GenericTypeArguments.First() != typeof(string) && perProperty.PropertyType.GenericTypeArguments.First() != typeof(string))
                                {
                                    ...
                                    if (perProperty.PropertyType.GetTypeInfo().GetInterface("IEnumerable", true) != null && dProperty.PropertyType.GetTypeInfo().GetInterface("IEnumerable", true) != null)
                                    {

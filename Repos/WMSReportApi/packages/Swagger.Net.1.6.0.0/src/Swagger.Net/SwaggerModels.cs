    ...
    public static class SwaggerGen
    {
        ...
        public static ResourceApi CreateResourceApi(ApiDescription api)
        {
            ...
            if (!api.ActionDescriptor.ActionName.EndsWith("API"))
            {
                ...
                {
                    if (path.Contains("?"))
                        path += "&OperationUserId={OperationUserId}";
                    else
                        path += "?OperationUserId={OperationUserId}";
                }
            }
            ...
            where T : MemberInfo
        {
            ...
            if (pi is MethodInfo)
            {
                ...
                var key = string.Format("/doc/members/member[@name='M:{0}.{1}{2}']/summary", belongToType.FullName, pi.Name, parms.Length > 0 ? "(" + string.Join(",", parms.Select(x => x.ParameterType.FullName)) + ")" : string.Empty);
                node = navi.SelectSingleNode(key);
            }
            else if (pi is PropertyInfo)
            {
                var key = string.Format("/doc/members/member[@name='P:{0}']/summary", belongToType.FullName + "." + pi.Name);
                node = navi.SelectSingleNode(key);
            }
            if (node != null)
        ...
        public static string GetDocumentation(ParameterInfo pi, Type belongToType = null)
        {
            if (belongToType == null)
            ...
            var key = string.Format("/doc/members/member[@name='M:{0}.{1}{2}']/param[@name='{3}']",
                belongToType.FullName, methodInfo.Name, parms.Length > 0 ? "(" + string.Join(",", parms.Select(x => x.ParameterType.FullName)) + ")" : string.Empty, pi.Name);
            node = navi.SelectSingleNode(key);
            //if (pi is MethodInfo)
            //{
            //    var parms = (pi as MethodInfo).GetParameters();
        ...
        public static ResourceApiOperationParameter CreateResourceApiOperationParameter(ResourceListing r, ApiDescription api, ApiParameterDescription param, XmlCommentDocumentationProvider docProvider)
        {
            ...
            if (param.ParameterDescriptor == null)
                throw new Exception(string.Format("{1}中的参数{0}不存在ParameterDescriptor信息，请确认", param.Name, api.RelativePath));
            ...
            ResourceApiOperationParameter parameter = new ResourceApiOperationParameter()
            {
                ...
                name = param.Name,
                description = GetParameterDescription((param.ParameterDescriptor as ReflectedHttpParameterDescriptor).ParameterInfo),//param.Name.Equals("sessionKey") ? "Login session" : (string.IsNullOrWhiteSpace(param.Documentation) ? NODESCRIPTION : param.Documentation),
                dataType = dataType,
        ...
        private static string GetParameterDescription(System.Reflection.ParameterInfo parameterInfo)
        {
            string desc;
            if (parameterInfo.Name.Equals("sessionKey", StringComparison.OrdinalIgnoreCase))
            {
                desc = "Login Session";
            }
            else
            {
                desc = GetDocumentation(parameterInfo);
            }

...
namespace FortuneLab.WebApiClient.Query
{
    ...
    public class CoreQuery
    {
        ...
        {
            CustomParameters.Add(string.Format("{0}={1}", HttpUtility.UrlEncode(key), HttpUtility.UrlEncode(value)));
        }
        ...
        public override string ToString()
        {
            ...
            var parameters = properties.ConvertAll(item => item.GetFormattedValue(this))
        ...
        private List<string> LoadParmsObj()
        {
            ...
            foreach (PropertyInfo propertyInfo in ParmsObj.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (obj is IQueryValueObject)
                {
                    reslovedValue = ((IQueryValueObject)obj).GetValue();
                }
                else
                {
                    reslovedValue = obj;
                ...
                {
                    value = HttpUtility.UrlEncode(reslovedValue.ToString());
                    if (string.IsNullOrWhiteSpace(value))
                        continue;//如果是NULL或者Empty, 不加入参数
                    value = string.Format("{0}={1}", HttpUtility.UrlEncode(propertyInfo.Name), value);
                }
        ...
        protected class PropertyDescription
        {
            ...
            public string GetFormattedValue(object current)
            {
                ...
                if (obj != null && Attributes.Count > 0)
                {
                    ...
                    value = HttpUtility.UrlEncode(string.Format(format, obj));
                    value = string.Format("{0}={1}", HttpUtility.UrlEncode(attribute.Name), value);
                }

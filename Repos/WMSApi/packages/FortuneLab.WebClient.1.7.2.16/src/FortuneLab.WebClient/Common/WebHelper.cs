...
namespace FortuneLab.WebClient.Common
{
    ...
    public partial class WebHelper
    {
        ...
        private string GetSiteHost(bool useSsl)
        {
            ...
            else
            {
                ...
                //result = siteSettings.SiteUrl;
                if (string.IsNullOrWhiteSpace(result))
                {
                    result = @"http://www.defaultwebsite.com";
                }
            }
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new Exception("needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        private bool TryWriteGlobalAsax()
        {
            try
            {
                //When a new plugin is dropped in the Plugins folder and is installed into system, 
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["flab.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["flab.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["flab.IsPOSTBeingDone"] = value;
            }
        }
